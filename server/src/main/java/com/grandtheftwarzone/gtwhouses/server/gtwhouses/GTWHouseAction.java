...
import com.grandtheftwarzone.gtwhouses.server.gtwhouses.actions.*;
...
public class GTWHouseAction {
    public static void handle(HouseActions action, Player player, String houseName, int... args) throws InvalidActionException {
        ...
        if (action.isRequiresHouse()) {
            house = GTWHouses.getHousesManager().getHouse(houseName);
        ...
        switch (action) {
            ...
            //Admin
            case Admin:
                GTWHouses.getPacketManager().sendPacket(player, new HousesGUIS2CPacket(OpenGUIC2SPacket.OpenGUIType.ADMIN_PANEL, GTWHouses.getHousesManager().getHouses()));
                break;
            case Register:
